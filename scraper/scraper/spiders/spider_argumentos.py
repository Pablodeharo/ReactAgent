...
class SpiderArgumentos(scrapy.Spider):
    ...
    def parse(self, response):
        # Extrae enlaces principales (aquellos que empiezan con 'azc')
        ...
        for enlace in enlaces_argumentos:
            url_completo = response.urljoin(enlace)
            yield scrapy.Request(url_completo, callback=self.parse_argumentos)
